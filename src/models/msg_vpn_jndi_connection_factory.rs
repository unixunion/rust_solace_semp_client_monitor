...
pub struct MsgVpnJndiConnectionFactory {
  ...
  /// The priority for receiving deliver-to-one (DTO) messages by the Subscriber (Consumer) if the messages are published on the local Router that the Subscriber is directly connected to.
  ...
  /// The priority for receiving deliver-to-one (DTO) messages by the Subscriber (Consumer) if the messages are published on a remote Router.
  ...
  /// Indicates whether a durable endpoint will be dynamically created on the Router when the client calls \"Session.createDurableSubscriber()\" or \"Session.createQueue()\". The created endpoint respects the message time-to-live (TTL) according to the \"dynamicEndpointRespectTtlEnabled\" property.
  ...
  /// The threshold for sending the acknowledgement (ACK) for guaranteed messages received by the Subscriber (Consumer) as a percentage of the \"guaranteedReceiveWindowSize\" value.
  ...
  /// The default delivery mode for messages sent by the Publisher (Producer). The allowed values and their meaning are:  <pre> \"persistent\" - Router spools messages (persists in the Message Spool) as part of the send operation. \"non-persistent\" - Router does not spool messages (does not persist in the Message Spool) as part of the send operation. </pre> 
  ...
  /// The ZLIB compression level for the connection to the Router. The value \"0\" means no compression, and the value \"-1\" means the compression level is specified in the JNDI Properties file.
  #[serde(rename = "transportCompressionLevel", skip_serializing_if="Option::is_none")]
  transport_compression_level: Option<i32>,
  /// The maximum number of retry attempts to establish an initial connection to the host (Router) or list of hosts (Routers). The value \"0\" means a single attempt (no retries), and the value \"-1\" means to retry forever.
  #[serde(rename = "transportConnectRetryCount", skip_serializing_if="Option::is_none")]
  transport_connect_retry_count: Option<i32>,
  /// The maximum number of retry attempts to establish an initial connection to each host (Router) on the list of hosts (Routers). The value \"0\" means a single attempt (no retries), and the value \"-1\" means to retry forever.
  ...
  /// The timeout for establishing an initial connection to the Router, in milliseconds.
  #[serde(rename = "transportConnectTimeout", skip_serializing_if="Option::is_none")]
  transport_connect_timeout: Option<i32>,
  /// Enable or disable usage of the Direct Transport mode for sending non-persistent messages. When disabled, the Guaranteed Transport mode is used.
  ...
  /// The maximum number of consecutive application-level keepalive messages sent without the Router response before the connection to the Router is closed.
  ...
  /// The connection port number on the Router for SMF clients. The value \"-1\" means the port is specified in the JNDI Properties file.
  #[serde(rename = "transportPort", skip_serializing_if="Option::is_none")]
  transport_port: Option<i32>,
  /// The timeout for reading a reply from the Router, in milliseconds.
  ...
  /// The maximum number of attempts to reconnect to the host (Router) or list of hosts (Routers) after the connection has been lost. The value \"-1\" means to retry forever.
  #[serde(rename = "transportReconnectRetryCount", skip_serializing_if="Option::is_none")]
  transport_reconnect_retry_count: Option<i32>,
  /// The amount of time before making another attempt to connect or reconnect to the host (Router) after the connection has been lost, in milliseconds.
